            ...
            <form action="{{ route('venda.update') }}" method="POST" id="formSale">
                ...
                <div class="row">
                    ...
                    <div class="col-md-4">
                        ...
                        <input required id="notaFiscal" name="notaFiscal" type="text" value="{{ $venda->notaFiscal }}" class="form-control">
                    </div>
                    <div class="col-md-4">
                        ...
                        <select class="form-control" name="cliente" required>
                            ...
                            @foreach ($clientes as $cliente)
                                <option value="{{ $cliente->id }}" @if($venda->idCliente == $cliente->id) selected @endif >{{ $cliente->nome }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-4">
                        ...
                        <select class="form-control" name="formaPagamento" required>
                            ...
                            @foreach ($formasPagamento as $formaPagamento)
                                <option value="{{ $formaPagamento->id }}" @if($venda->idFormaPagamento == $formaPagamento->id) selected @endif>{{ $formaPagamento->descricao }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <table id="myTable">
                        ...
                        <tbody id="tbody">
                            ...
                            <tr id="produto_{{$key}}" class="p-2">
                                <th class="p-1 pr-4">
                                    <select required onchange="mudaProduto(1)" id="select_{{$key}}" class="form-control" name="produto[]">
                                        <option value="">Selecione o produto</option>  
                                        @foreach ($produtos as $produto)
                                            <option @if($produtoVenda->idProduto == $produto->id) selected @endif data-valor-unitario="{{ $produto->valorVenda }}" value="{{ $produto->id }}">{{ $produto->nome }}</option>
                                        @endforeach
                                    </select>
                                </th>
                                <th class="p-1">
                                    <input required onchange="mudaQuantidade({{$key}})" value="{{$produtoVenda->quantidade}}" id="quantidade_{{$key}}" name="quantidade[]" type="number" class="form-control">
                                </th>
                                <th class="p-1">
                                ...
                                <th><button onclick="javascript:removerProduto({{$key}})" type="button" class="btn">Remover</button></th>
                            </tr>
                            @endforeach
                        </tbody>
                    </table>
